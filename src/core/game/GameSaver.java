...
class GameSaver {
    ...
    static void writeTurnFile(GameState gs, Board gameBoard, long seed) {
        try{
            ...
            // JSON
            JSONObject game = buildGameJson(gs, gameBoard, seed);
    ...
    private static JSONObject buildGameJson(GameState gs, Board gameBoard, long seed) {
        ...
        for(Unit u: unitList){
            JSONObject uInfo = getUnitInfo(u);
        ...
        for(City c: citiesList){
            // City INFO: id:{x, y, tribeId, population_need, bound, level, isCapital, population,
            //                production, hasWalls, pointsWorth, building(array)}
            JSONObject cInfo = getCityInfo(c);
        ...
        for(Tribe t: tribes){
            JSONObject tribeInfo = getTribeInfo(t);
    ...
    private static JSONObject getTribeInfo(Tribe t) {
    ...
    private static JSONObject getCityInfo(City c) {
        ...
        // Save Buildings INFO
        JSONArray buildingList = getBuildingInfo(c);
        cInfo.put("buildings", buildingList);
        cInfo.put("units", c.getUnitsID());
        return cInfo;
    }

    private static JSONArray getBuildingInfo(City c) {
    ...
    private static JSONObject getUnitInfo(Unit u) {
